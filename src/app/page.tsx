...
const MEMBERS = [
  { name: "HUDDY", handle: "@huddy_lg", role: "Co-Founder", bio: "The creative mastermind", socials: { tiktok: "https://www.tiktok.com/@huddy_lg", instagram: "https://www.instagram.com/huddy_lg/", youtube: "https://www.youtube.com/@Huddy_lg" }, color: "#FFB21A", accent: "#C81E2A" },
  { name: "JVHN SEO", handle: "@jvhnseo", role: "Co-Founder", bio: "Viral content creator", socials: { tiktok: "https://www.tiktok.com/@jvhnseo", instagram: "https://www.instagram.com/jvhnseo/", youtube: "https://www.youtube.com/@JvhnSeo" }, color: "#C81E2A", accent: "#FFB21A" },
  { name: "BRANDON DELUNA", handle: "@djfashionkill", role: "Co-Founder", bio: "The style icon", socials: { tiktok: "https://www.tiktok.com/@djfashionkill", instagram: "https://www.instagram.com/brandondeluna_/" }, color: "#FFB21A", accent: "#C81E2A" },
  { name: "JGGLS", handle: "@jggls", role: "Co-Founder", bio: "Comedy king", socials: { tiktok: "https://www.tiktok.com/@jggls", instagram: "https://www.instagram.com/_jggls_/", youtube: "https://www.youtube.com/@jgglsofficial" }, color: "#C81E2A", accent: "#FFB21A" },
];
...
function MemberCard({ member, index }: { member: typeof MEMBERS[0]; index: number }) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 50 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: index * 0.15, ease: [0.22, 1, 0.36, 1] }} whileHover={{ y: -12, scale: 1.02 }} className="member-card bg-[#0f0f0f] p-8 relative overflow-hidden group cursor-pointer" style={{ borderColor: member.color }}>
      <div className="relative z-10">
        <motion.div className="flex justify-center mb-6" whileHover={{ scale: 1.1 }} transition={{ duration: 0.4 }}>
          <div className="w-28 h-28 border-4 flex items-center justify-center text-3xl font-bold" style={{ borderColor: member.color, boxShadow: `6px 6px 0 ${member.accent}`, backgroundColor: "#1a1a1a", color: member.color }}>{initials}</div>
        </motion.div>
        <div className="text-center mb-4">
          ...
          <p className="text-gray-500 text-sm mt-3 italic">{member.bio}</p>
        </div>
...
function MarqueeText() {
  return (
    <div className="overflow-hidden py-6 bg-[#FFB21A]">
...
function ContentCard({ title, platform, link, color, delay }: { title: string; platform: string; link: string; color: string; delay: number }) {
  return (
    <motion.a href={link} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay }} whileHover={{ scale: 1.05, y: -5 }} className="retro-border bg-[#0f0f0f] p-8 text-center flex flex-col items-center group relative overflow-hidden">
      <div className="relative z-10">
    ...
    <div className="min-h-screen bg-[#0a0a0a] overflow-x-hidden">
      <motion.header initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="fixed top-0 left-0 right-0 z-50 bg-[#0a0a0a]/95 backdrop-blur-md border-b-2 border-[#FFB21A]">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <motion.div className="text-xl md:text-2xl font-bold tracking-widest" whileHover={{ scale: 1.05 }}>
      ...
      <section ref={heroRef} className="min-h-screen flex items-center justify-center relative overflow-hidden pt-20">
        ...
        <motion.div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#FFB21A]/20 rounded-full blur-3xl" animate={{ scale: [1, 1.2, 1], opacity: [0.2, 0.3, 0.2] }} transition={{ duration: 4, repeat: Infinity }} />
        <motion.div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-[#C81E2A]/20 rounded-full blur-3xl" animate={{ scale: [1.2, 1, 1.2], opacity: [0.3, 0.2, 0.3] }} transition={{ duration: 4, repeat: Infinity, delay: 2 }} />
      ...
      <section id="about" className="py-32 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center">
            ...
            <p className="text-lg text-gray-400 leading-relaxed">With millions of followers across all platforms, we are building something special. Join the movement and become part of the DERF family.</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[{ label: "Founded", value: "2024" }, { label: "Members", value: "4" }, { label: "Platforms", value: "3+" }, { label: "Vibes", value: "100%" }].map((stat, i) => (
              <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="retro-border bg-[#0f0f0f] p-6">
      ...
      <section id="team" className="py-32 px-4 bg-[#0f0f0f]">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">{MEMBERS.map((member, index) => <MemberCard key={member.handle} member={member} index={index} />)}</div>
        </div>
      </section>
      ...
      <section id="contact" className="py-32 px-4 bg-[#0f0f0f]">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-xl text-gray-300 mb-12">For business inquiries, collaborations, or just to say what is up - hit us up.</motion.p>
          <motion.a href="mailto:derfboys@gmail.com" initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="arcade-btn text-xl inline-block">derfboys@gmail.com</motion.a>
        </div>
      </section>

      <footer className="border-t-2 border-[#FFB21A] py-12 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <motion.div className="text-2xl font-bold tracking-widest" whileHover={{ scale: 1.05 }}>
              <span className="text-[#FFB21A]">THE</span> <span className="text-white">DERF</span> <span className="text-[#C81E2A]">BOYS</span>
            </motion.div>
            <div className="flex gap-6">
              <motion.a href="https://www.tiktok.com/@jggls" target="_blank" rel="noopener noreferrer" className="text-gray-400 p-2" whileHover={{ scale: 1.2, color: "#FFB21A" }}><TikTokIcon /></motion.a>
              <motion.a href="https://www.instagram.com/_jggls_/" target="_blank" rel="noopener noreferrer" className="text-gray-400 p-2" whileHover={{ scale: 1.2, color: "#C81E2A" }}><InstagramIcon /></motion.a>
              <motion.a href="https://www.youtube.com/@jgglsofficial" target="_blank" rel="noopener noreferrer" className="text-gray-400 p-2" whileHover={{ scale: 1.2, color: "#FF0000" }}><YouTubeIcon /></motion.a>
            </div>
            <p className="text-gray-500 text-sm">The Derf Boys. Do Everything Really Fun.</p>
          </div>
        </div>
      </footer>
    </div>
